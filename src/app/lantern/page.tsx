...
function LanternIcon({ size = 20, className = "" }: { size?: number; className?: string }) {
  return (
    ...
    <div className="relative mx-auto w-full max-w-[500px] overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-xl shadow-amber-100/60">
      ...
      <div className="flex items-center justify-between border-b border-gray-100 px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          {["Overview","Signals","Risks","Docs"].map((tab, i) => (
            <span key={tab} className={`text-[10px] font-medium ${i === 0 ? "text-amber-600 underline underline-offset-2 decoration-amber-300" : "text-gray-400"}`}>
              {tab}
            </span>
          ))}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="mb-4 flex items-start justify-between">
          ...
          <div className="flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1">
            ...
            <div className="ml-1 flex gap-0.5">
              {[1,2,3,4].map(i => <span key={i} className="h-1.5 w-1.5 rounded-full bg-amber-400" />)}
        ...
        <div className="mt-2.5 flex items-center gap-2 rounded-xl border border-gray-100 bg-white px-3 py-2">
          ...
          <p className="text-[10px] text-gray-400">AI agent brief <span className="font-semibold text-gray-600">ready for Claude &amp; Cursor</span></p>
